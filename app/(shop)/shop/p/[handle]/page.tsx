...
export default async function ProductPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const p = await getStorefrontProduct(handle).catch(() => null);
  if (!p) notFound();
      ...
      <div className="mt-4 grid gap-10 lg:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-3 flex items-baseline gap-3">
            <span className="text-2xl font-bold text-neutral-900">£{Number(p.price).toFixed(2)}</span>
            {p.compareAt && <span className="text-lg text-neutral-400 line-through">£{Number(p.compareAt).toFixed(2)}</span>}
            {p.compareAt && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-xs font-bold text-amber-700">SALE</span>}
          </div>
